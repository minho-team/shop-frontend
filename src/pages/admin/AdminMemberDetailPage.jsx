...
const AdminMemberDetailPage = () => {
    ...
    const orderEndPage = Math.min(orderStartPage + PAGE_GROUP - 1, orderTotalPages);
            ...
            <AdminLayout pageTitle="회원 상세">
                ...
                <div style={card}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", borderBottom: "2px solid #222", paddingBottom: "8px" }}>
                        <h3 style={{ fontSize: "15px", fontWeight: "bold", color: "#222", margin: 0 }}>장바구니 현황</h3>
                        <span style={{ fontSize: "13px", color: "#888" }}>총 {cartItems.length}종류</span>
                    </div>

                    {cartLoading ? (
                        <p style={{ textAlign: "center", padding: "20px", color: "#999" }}>로딩 중...</p>
                    ) : cartItems.length === 0 ? (
                        <p style={{ fontSize: "13px", color: "#aaa", padding: "16px 0" }}>장바구니가 비어있습니다.</p>
                    ) : (
                        cartItems.map((item) => (
                            <div key={item.cartItemNo} style={{ borderBottom: "1px solid #eee" }}>
                                ...
                                <div
                                    ...
                                    style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 8px", cursor: "pointer" }}
                                    onMouseEnter={(e) => e.currentTarget.style.background = "#fafafa"}
                                    onMouseLeave={(e) => e.currentTarget.style.background = cartExpandedMap[item.cartItemNo] ? "#fafafa" : "white"}>
                                    <span style={{ fontSize: "12px", color: "#aaa", width: "16px", flexShrink: 0 }}>
                                        {cartExpandedMap[item.cartItemNo] ? "▼" : "▶"}
                                    </span>
                                    {/* 상품명 */}
                                    <span style={{ fontSize: "14px", color: "#222", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                        {item.productName}
                                    </span>
                                    {/* 수량 */}
                                    <span style={{ fontSize: "13px", color: "#666", whiteSpace: "nowrap", flexShrink: 0 }}>
                                        {item.quantity}개
                                    </span>
                                    {/* 가격 */}
                                    <span style={{ fontSize: "13px", color: "#222", fontWeight: "bold", whiteSpace: "nowrap", flexShrink: 0 }}>
                                ...
                                {cartExpandedMap[item.cartItemNo] && (
                                    <div style={{ background: "#f9f9f9", padding: "14px 24px", borderTop: "1px solid #eee" }}>
                                        <div style={{ display: "flex", gap: "24px", fontSize: "13px", color: "#666", flexWrap: "wrap" }}>
                                            <span>상품번호: <b style={{ color: "#222" }}>{item.productNo}</b></span>
                                            <span>사이즈: <b style={{ color: "#222" }}>{item.optionSize || "-"}</b></span>
                                            <span>색상: <b style={{ color: "#222" }}>{item.color || "-"}</b></span>
                                            <span>수량: <b style={{ color: "#222" }}>{item.quantity}개</b></span>
                                            <span>정가: <b style={{ color: "#222" }}>{formatPrice(item.price)}</b></span>
                                            {item.salePrice && (
                                                <span>할인가: <b style={{ color: "#c62828" }}>{formatPrice(item.salePrice)}</b></span>
                                            )}
                                        </div>
                                    </div>
                                )}
                ...
                <div style={card}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", borderBottom: "2px solid #222", paddingBottom: "8px" }}>
                        <h3 style={{ fontSize: "15px", fontWeight: "bold", color: "#222", margin: 0 }}>주문 내역</h3>
                        <span style={{ fontSize: "13px", color: "#888" }}>총 {orderTotalCount}건</span>
                    </div>

                    {orderLoading ? (
                        <p style={{ textAlign: "center", padding: "20px", color: "#999" }}>로딩 중...</p>
                    ) : orderList.length === 0 ? (
                        <p style={{ fontSize: "13px", color: "#aaa", padding: "16px 0" }}>주문 내역이 없습니다.</p>
                    ) : (
                        <>
                            {orderList.map((order) => (
                                <div key={order.orderNo} style={{ borderBottom: "1px solid #eee" }}>
                                    ...
                                    <div
                                        ...
                                        {/* 총금액 */}
                                        <span style={{ flex: 1, fontSize: "14px", color: "#222", fontWeight: "bold" }}>
                                            {formatPrice(order.totalPrice)}
                                        </span>
                                        {/* 환불상태 */}
                                        {order.refundStatus && (
                                            <span style={{ fontSize: "12px", color: "#c62828", whiteSpace: "nowrap", flexShrink: 0 }}>
                                                {order.refundStatus}
                                            </span>
                                        )}
                                        {/* 주문일 */}
                                        <span style={{ fontSize: "12px", color: "#aaa", whiteSpace: "nowrap", flexShrink: 0 }}>
                                    ...
                                    {orderExpandedMap[order.orderNo] && (
                                        <div style={{ background: "#f9f9f9", padding: "14px 24px", borderTop: "1px solid #eee" }}>
                                            <div style={{ display: "flex", gap: "20px", fontSize: "13px", color: "#666", flexWrap: "wrap", marginBottom: "10px" }}>
                                                <span>주문자: <b style={{ color: "#222" }}>{order.ordererName}</b></span>
                                                <span>주문일: <b style={{ color: "#222" }}>{formatDateTime(order.createdAt)}</b></span>
                                            </div>
                                            <div style={{ display: "flex", gap: "20px", fontSize: "13px", color: "#666", flexWrap: "wrap" }}>
                                                <span>수령자: <b style={{ color: "#222" }}>{order.receiverName}</b></span>
                                                <span>연락처: <b style={{ color: "#222" }}>{formatPhone(order.receiverPhoneNumber)}</b></span>
                                            </div>
                                            <div style={{ fontSize: "13px", color: "#666", marginTop: "6px" }}>
                                                배송지: <b style={{ color: "#222" }}>
                                                    [{order.receiverZipCode}] {order.receiverBaseAddress} {order.receiverDetailAddress}
                                                </b>
                                            </div>
                                            {order.message && (
                                                <div style={{ fontSize: "13px", color: "#666", marginTop: "6px" }}>
                                                    배송 메시지: <b style={{ color: "#222" }}>{order.message}</b>
                                                </div>
                                            )}
                                            {/* 주문 상세 페이지 이동 버튼 */}
                                            <button
                                                onClick={() => navigate(`/admin/order/detail/${order.orderNo}`)}
                            ...
                            {orderTotalPages > 1 && (
                                <div style={{ display: "flex", justifyContent: "center", gap: "6px", marginTop: "16px" }}>
                                    <button onClick={() => setOrderPage(orderStartPage - 1)} disabled={orderStartPage === 1}
                                        style={{ ...pageBtn, color: orderStartPage === 1 ? "#ccc" : "#333" }}>&lt;</button>
                                    {Array.from({ length: orderEndPage - orderStartPage + 1 }, (_, i) => orderStartPage + i).map((p) => (
                                        <button key={p} onClick={() => setOrderPage(p)}
                                            style={{ ...pageBtn, background: orderPage === p ? "#222" : "#fff", color: orderPage === p ? "#fff" : "#333" }}>
                                            {p}
                                        </button>
                                    ))}
                                    <button onClick={() => setOrderPage(orderEndPage + 1)} disabled={orderEndPage === orderTotalPages}
                                        style={{ ...pageBtn, color: orderEndPage === orderTotalPages ? "#ccc" : "#333" }}>&gt;</button>
                                </div>
                            )}
                        </>
                    )}
                ...
                <div style={card}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", borderBottom: "2px solid #222", paddingBottom: "8px" }}>
                        <h3 style={{ fontSize: "15px", fontWeight: "bold", color: "#222", margin: 0 }}>1:1 문의 내역</h3>
                        <span style={{ fontSize: "13px", color: "#888" }}>총 {inqTotalCount}건</span>
                    </div>

                    {inqLoading ? (
                        <p style={{ textAlign: "center", padding: "20px", color: "#999" }}>로딩 중...</p>
                    ) : inquiryList.length === 0 ? (
                        <p style={{ fontSize: "13px", color: "#aaa", padding: "16px 0" }}>문의 내역이 없습니다.</p>
                    ) : (
                        <>
                            {inquiryList.map((inq) => (
                                <div key={inq.inquiryNo} style={{ borderBottom: "1px solid #eee" }}>
                                    ...
                                    <div
                                        ...
                                        <span style={{ fontSize: "12px", color: "#666", background: "#f0f0f0", padding: "2px 8px", borderRadius: "10px", whiteSpace: "nowrap", flexShrink: 0 }}>
                                            {inq.category}
                                        </span>
                                        <span style={{ fontSize: "14px", color: "#222", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                            {inq.title}
                                    ...
                                    {/* 펼쳐진 상세 */}
                                    {inqExpandedMap[inq.inquiryNo] && (
                                        <div style={{ background: "#f9f9f9", padding: "16px 24px", borderTop: "1px solid #eee" }}>
                                            <div style={{ display: "flex", gap: "20px", fontSize: "12px", color: "#888", marginBottom: "12px", flexWrap: "wrap" }}>
                                                <span>문의번호: <b style={{ color: "#444" }}>{inq.inquiryNo}</b></span>
                                                <span>카테고리: <b style={{ color: "#444" }}>{inq.category}</b></span>
                                                <span>비밀글: <b style={{ color: "#444" }}>{inq.secretYn === "Y" ? "🔒 비밀글" : "공개"}</b></span>
                                                <span>조회수: <b style={{ color: "#444" }}>{inq.viewCount}</b></span>
                                                <span>등록일: <b style={{ color: "#444" }}>{formatDateTime(inq.createdAt)}</b></span>
                                                {inq.updatedAt && <span>수정일: <b style={{ color: "#444" }}>{formatDateTime(inq.updatedAt)}</b></span>}
                                            </div>
                            ...
                            {inqTotalPages > 1 && (
                                <div style={{ display: "flex", justifyContent: "center", gap: "6px", marginTop: "16px" }}>
                                    <button onClick={() => setInqPage(inqStartPage - 1)} disabled={inqStartPage === 1}
                                        style={{ ...pageBtn, color: inqStartPage === 1 ? "#ccc" : "#333" }}>&lt;</button>
                                    {Array.from({ length: inqEndPage - inqStartPage + 1 }, (_, i) => inqStartPage + i).map((p) => (
                                        <button key={p} onClick={() => setInqPage(p)}
                                            style={{ ...pageBtn, background: inqPage === p ? "#222" : "#fff", color: inqPage === p ? "#fff" : "#333" }}>
                                            {p}
                                        </button>
                                    ))}
                                    <button onClick={() => setInqPage(inqEndPage + 1)} disabled={inqEndPage === inqTotalPages}
                                        style={{ ...pageBtn, color: inqEndPage === inqTotalPages ? "#ccc" : "#333" }}>&gt;</button>
                                </div>
                            )}
                        </>
                    )}
