...
const OrderHistory = ({ user }) => {
  ...
  if (loading)
    return <div className="loading-container">데이터 로드 중...</div>;
...
const ReviewEditModal = ({ review, onClose, onUpdateSuccess }) => {
  ...
  const [previewUrl, setPreviewUrl] = useState(
    review.imageUrl ? `/upload/${review.imageUrl}` : null,
  );
          ...
          <div className="edit-form-group">
            ...
            {previewUrl && (
              <img
                src={previewUrl}
                alt="Preview"
                style={{ width: "80px", marginTop: "10px", display: "block" }}
              />
            )}
          </div>
...
const WishListHistory = ({ user }) => {
  ...
  if (loading)
    return <div className="loading-container">찜 목록 로딩 중...</div>;
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
        {[
          ...
          {
            key: "UNUSED",
            label: `미사용 (${coupons.filter((c) => c.usedYn === "N").length})`,
          },
          {
            key: "USED",
            label: `사용완료 (${coupons.filter((c) => c.usedYn === "Y").length})`,
          },
        ].map((f) => (
          <button
            key={f.key}
...
const MyPage = () => {
  ...
  const getProgress = () => {
    ...
    const segmentProgress =
      ((purchaseCount - prevTarget) / (nextTarget - prevTarget)) * 100;
  ...
  if (userLoading)
    return <div className="loading-container">정보 로딩 중...</div>;
      ...
      <div className="user-summary-bg">
        <section className="user-summary">
          ...
          <div className="grade-badge">
            ...
            <div
              className="grade-display-wrapper"
